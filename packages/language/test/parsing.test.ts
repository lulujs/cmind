...
import type { MindMap, TemplateMetadata, ThemeMetadata } from "../src/generated/ast.js";
import { createCmindServices, isMindMap, isPriorityAttribute, isProgressAttribute, isBoldAttribute, isItalicAttribute, isTemplateMetadata, isThemeMetadata } from "../src/index.js";
...
- child2`);
            ...
            expect(document.parseResult.value.root.children.length).toBeGreaterThanOrEqual(1);
        });
...
- child with spaces`);
            ...
            expect(checkDocumentValid(document)).toBeUndefined();
            expect(document.parseResult.value.root.children[0].text).toBe('child with spaces');
        });
    });
...
# Topic`);
            ...
            expect(isTemplateMetadata(document.parseResult.value.metadata[0])).toBe(true);
            if (isTemplateMetadata(document.parseResult.value.metadata[0])) {
                expect((document.parseResult.value.metadata[0] as TemplateMetadata).value).toBe('right');
            }
        });
...
# Topic`);
            ...
            expect(checkDocumentValid(document)).toBeUndefined();
            expect(document.parseResult.value.metadata).toHaveLength(1);
            expect(isThemeMetadata(document.parseResult.value.metadata[0])).toBe(true);
            if (isThemeMetadata(document.parseResult.value.metadata[0])) {
                expect((document.parseResult.value.metadata[0] as ThemeMetadata).value).toBe('fresh-blue');
            }
        });
