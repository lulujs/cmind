...
const validProgressArb = fc.oneof(
  fc.constant(undefined), // @progress without value
  fc.integer({ min: 2, max: 8 }).map((n) => n.toString().padStart(2, "0")), // @progress(02-08)
);
...
const attributeArb: fc.Arbitrary<AttributeData> = fc.oneof(
  ...
  validProgressArb.map((v) => ({ type: "progress" as const, value: v })),
);

const uniqueAttributesArb = fc
  ...
  return sorted
    .map((a) => {
      switch (a.type) {
        ...
        case "progress":
          return a.value ? `@progress(${a.value})` : `@progress`;
        case "bold":
          return "@bold";
        case "italic":
          return "@italic";
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (text, priority, progress) => {
          ...
          const attrs1 =
            ast1.root.children[0]?.attributes.map(extractAttributeData) ?? [];
          const attrs2 =
            ast2.root.children[0]?.attributes.map(extractAttributeData) ?? [];
