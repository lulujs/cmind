    ...
    try {
      ...
      if (fileExtension !== ".cmind") {
        const detailedError = this.errorHandler.processError(
          new Error(
            `Invalid file extension. Expected .cmind, got ${fileExtension}`,
          ),
      ...
      const resolvedOutputDir =
        outputDir || this.configManager.getResolvedOutputDirectory(filePath);

      // Generate KityMinder KM file
      const outputPath = await this.generateOutput(
        model,
        filePath,
        resolvedOutputDir,
      );
      ...
      if (recovered) {
        ...
        try {
          ...
          const resolvedOutputDir =
            outputDir ||
            this.configManager.getResolvedOutputDirectory(filePath);
          const outputPath = await this.generateOutput(
            model,
            filePath,
            resolvedOutputDir,
          ...
          const retryDetailedError = this.errorHandler.processError(
            retryError,
            {
              filePath,
    ...
    try {
      ...
      if (!extensions.includes(path.extname(filePath))) {
        const error = new SyntaxError(
          `Invalid file extension. Expected one of: ${extensions.join(", ")}, but got: ${path.extname(filePath)}`,
        );
        ...
        throw new SyntaxError(
          this.errorHandler.formatUserMessage(processedError),
        );
      }
      ...
      if (lexerErrors.length > 0) {
        ...
        const combinedMessage = detailedErrors
          .map((e) => this.errorHandler.formatUserMessage(e))
          .join("\n");
      ...
      if (!parseResult) {
        const error = new SyntaxError(
          "Failed to parse document: No parse result available",
        );
        const processedError = this.errorHandler.processError(error, {
          filePath,
