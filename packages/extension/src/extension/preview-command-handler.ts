...
export class PreviewCommandHandler {
    ...
    try {
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          progress.report({
            increment: 0,
            message: "Creating preview panel...",
          });
          ...
          this.syncStates.set(
            targetFile,
            this.configurationManager.isPreviewAutoUpdateEnabled(),
          );
  ...
  private getActivePreviewFile(): string | undefined {
    ...
    if (activeEditor) {
      const activeFile = activeEditor.document.uri.fsPath;
      if (
        this.isValidCmindFile(activeFile) &&
        this.previewPanelManager.isPreviewOpen(activeFile)
      ) {
        return activeFile;
      }
    }
