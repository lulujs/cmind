...
export function deactivate(): Thenable<void> | undefined {
    ...
    try {
        ...
        if (configurationManager && 'dispose' in configurationManager && typeof (configurationManager as any).dispose === 'function') {
 ...
 */
function setupConfigurationChangeHandling(context: vscode.ExtensionContext): void {
    ...
    const configChangeDisposable = configurationManager.onConfigurationChanged((newConfig) => {
        ...
        try {
            ...
            if (memoryCacheService) {
                memoryCacheService.updateMaxMemoryUsage(newConfig.preview.maxMemoryUsage);
            }
...
function setupFileLifecycleHandling(context: vscode.ExtensionContext): void {
    ...
    const fileDeleteWatcher = vscode.workspace.createFileSystemWatcher('**/*.cmind');
    ...
    const documentCloseDisposable = vscode.workspace.onDidCloseTextDocument((document) => {
        if (document.languageId === 'cmind' || document.fileName.endsWith('.cmind')) {
            ...
            try {
                ...
                console.error(`Error handling document close for ${filePath}:`, error);
            }
        }
    });
